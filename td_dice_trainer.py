...
class TdDiceTrainer(object):
    def __init__(
        ...
        self.actor = actor
        self.critic = critic
        self.critic_target_ref = critic_target
        ...
        if not self.training_penalty:
            ...
            utils.print_banner(f"Using penalized training in TdDiceTrainer !!! With g_constraint={self.dr_optimizer.g_constraint}")
        ...
        utils.print_banner(f"Initialize TdDiceTrainer !!! training_penalty={self.training_penalty}, beta={self.beta}, loss_type={self.loss}, weight_decay={self.dr_optimizer.__dict__['param_groups'][0]['weight_decay']}")

    def train_OPE(self, iterations, batch_size=256, record_training_stats=False):
        # initialize critic_target before each training of dr-model
        critic_target = copy.deepcopy(self.critic_target_ref)
        ...
        for idx in range((int(iterations))):
            ...
            with torch.no_grad():
                ...
                init_Q = critic_target.q1(start_state, start_action)
                ...
                target = start_Q + self.discount * torch.mean((1. - batch['terminals']) * critic_target.q1(batch['next_observations'], next_action) * w_sa_target)
            ...
            if self.training_penalty:
                ...
                dr_loss = self.loss((w_sa * self.critic.q1(batch['observations'], batch['actions'])).mean(), target)
                dr_loss.backward()
                # Gradient correction
            ...
            for param, target_param in zip(self.critic.parameters(), critic_target.parameters()):
                target_param.data.copy_(self.beta * param.data + (1. - self.beta) * target_param.data)
