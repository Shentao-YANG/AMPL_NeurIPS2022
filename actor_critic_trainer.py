...
class GanACTrainer(object):
    ...
    def __init__(
        ...
        self.remove_reg = remove_reg

        print_banner(f"reward_max: {self.reward_max:.3f}, reward_min: {self.reward_min:.3f}")
        print_banner(f"Initialized Model-based GAN Actor-Critic Trainer, remove_reg={self.remove_reg} !")
    ...
    def get_true_fake_samples(self, n_real_data, n_generated_data):
        batch_fake_state = self.sample_batch(n_real_data=n_real_data, n_generated_data=n_generated_data)['observations']
        ...
        fake_state_repeat, fake_action_samples, fake_raw_action_samples = \
            self.actor.sample_multiple_actions(batch_fake_state, num_action=1, std=self.state_noise_std, return_raw_action=True)
        with torch.no_grad():
        ...
        fake_not_dones = (self.dynamics_model.termination(fake_state_repeat, fake_action_samples, fake_next_state, fake_rewards).squeeze() < 0.5)  # check whether fake_next_state is terminal state
        ...
        batch_truth = self.sample_batch(n_real_data=fake_samples.shape[0], n_generated_data=0)
        true_samples = torch.cat(
            [
                batch_truth['observations'], self.actor.pre_scaling_action(batch_truth['actions'])
            ], dim=1
        )

        return fake_samples, true_samples

    def get_generator_and_discriminator_loss(self, fake_samples, true_samples):
    ...
    def logging_common(self, warm_start, n_real_data, n_generated_data, true_samples, target_Q, critic_loss, epoch_critic_loss, policy_loss, generator_loss, Q_values, real_loss, fake_loss, discriminator_loss, current_Q1, rollout_len, generated_rewards, terminated):
    ...
    def train_from_torch(self, iterations):
        ...
        for _ in range(iterations):
            ...
            """
            fake_samples, true_samples = self.get_true_fake_samples(n_real_data=n_real_data, n_generated_data=n_generated_data)

            generator_loss, discriminator_loss, real_loss, fake_loss, additional_logging = self.get_generator_and_discriminator_loss(fake_samples=fake_samples, true_samples=true_samples)
        ...
        self.logging_common(warm_start, n_real_data, n_generated_data, true_samples, target_Q, critic_loss, epoch_critic_loss, policy_loss, generator_loss, Q_values, real_loss, fake_loss, discriminator_loss, current_Q1, rollout_len, generated_rewards, terminated)
